...
async function enrichMissingImages(items){
  const need = items.filter(i=>!i.image && i.link);
  for(let i=0;i<need.length;i+=concurrentFetches){
    const batch = need.slice(i,i+concurrentFetches);
